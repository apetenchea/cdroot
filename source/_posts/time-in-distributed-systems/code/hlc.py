...
class HLC:
    ...
    def get_timestamp(self):
        ...
        if physical <= self._last_timestamp.physical:
            # Physical time stays the same, but the logical component gets updated.
    ...
    def get_timestamp_and_adjust(self, received: Timestamp):
        ...
        """
        physical = max(self.get_physical_time(), self._last_timestamp.physical, received.physical)
        if physical == self._last_timestamp.physical:
            if physical == received.physical:
                # The last recorded timestamp and the received timestamp have the same physical time,
                # greater than the current physical time.
                logical = max(self._last_timestamp.logical, received.logical) + 1
            else:
                # The last recorded timestamp has a higher physical time than the received timestamp.
                logical = self._last_timestamp.logical + 1
        else:
            if physical == received.physical:
                # The received timestamp has a higher physical time than the last recorded timestamp.
                logical = received.logical + 1
            else:
                # The current physical time is higher than both the last recorded timestamp and the received timestamp.
                logical = 0
        self._last_timestamp = Timestamp(physical, logical)
        return self._last_timestamp
