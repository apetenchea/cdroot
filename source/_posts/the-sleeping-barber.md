...
1. **The barber may fall asleep while there are customers in the waiting room.** Imagine a customer walks in to
    ...
    customer will sit down waiting indefinitely for the barber to invite him into the working room.
...
When customers arrive, they execute the `customer` function. The customer does not have to loop, as it only needs to
...
that any modifications to the `waiting` and `bs` variables happen atomically.
...
`true` for all of them. There is no guarantee that between the comparison and the update, other customers won't
...
It reports a data-race involving two threads, _T1_ and _T2_, both trying to modify the same global location, _shared_data_.
...
global variable).  
Implementing a fix is simple, as we just have to declare _shared_data_ as `std::atomic_int`, and the warning goes away.
